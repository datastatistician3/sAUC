...
fasd_label[, c("x1", "x2", "group")] <- lapply(fasd_label[, c("x1", "x2", "group")], function(x) factor(x))

sAUC::sAUC(x = y ~ x1, treatment_group = "group", data = fasd_label)

sAUC::sAUC(x = y ~ x1 + x2, treatment_group = "group", data = fasd_label)
